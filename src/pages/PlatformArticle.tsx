...
const PlatformArticle: React.FC = () => {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Squirrelll.ing', item: 'https://squirrelll.ing/squirrelll' },
      ...(category ? [{ '@type': 'ListItem', position: 3, name: category.name, item: `https://squirrelll.ing/squirrelll/category/${category.slug}` }] : []),
  ...
  return (
    <KnowledgeArticle
      ...
      node={p}
      prev={prev ? { path: `/squirrelll/${prev.slug}`, title: prev.title } : undefined}
      next={next ? { path: `/squirrelll/${next.slug}`, title: next.title } : undefined}
    />
  );
};
